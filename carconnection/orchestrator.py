...
def getConPopen(brand):
    print(f"created consumer for {brand}")
    return subprocess.Popen(["/home/rejk/Downloads/prj/dsproject/venv/bin/python","consumer.py","-b",f"{brand}"])
def getPopen():
    ...
    for brand in BRANDS:
        ...
        popen= subprocess.Popen(["/home/rejk/Downloads/prj/dsproject/venv/bin/python","producer.py","-b",f"{brand}"])
        Timer(20,getConPopen,args=[brand]).start()
        yield popen
genPopen=getPopen()
